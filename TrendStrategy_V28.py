...
在 V2.7 上最小增量：Final Regime + Double MA 真交叉之后，
同一根 K 线再用 MACD Histogram ∧ Volume Structure 确认新开仓。
不覆盖 V23 / V24 / V241 / V25 / V26 / V27。
不改 execution_core.py / wqk_ctp_gateway.py。不改 V2.7 Regime / V2.5 Exit Engine。

Final Regime 只过滤新开仓，不负责平仓。出场仍只 set_target_pos(0) + drive_execution。
...
def array_manager_size(
    ...
    adx_period: int = 14,
    bb_period: int = 26,
    bb_percentile_lookback: int = 120,
) -> int:
    """均线 / ATR / ADX / BB 百分位预热取更大者。TA-Lib ADX(n) 约需 2*n+1 根。"""
    ...
    bb_need = max(int(bb_period or 1), 1) + max(int(bb_percentile_lookback or 1), 1) - 1
    return max(ma_need, atr_need, adx_need, bb_need)
...
# ============================================================
# V2.7 Regime：BB + MA Structure + Final Regime（只过滤 Entry）
# ============================================================
...
BB_COMPRESSION_HOLD = "COMPRESSION_HOLD"
BB_COMPRESSION_RELEASE = "COMPRESSION_RELEASE"
BB_NEUTRAL = "NEUTRAL"
BB_EXPANSION = "EXPANSION"
BB_UNAVAILABLE = "UNAVAILABLE"

BB_DIR_BULL = "BULL"
BB_DIR_BEAR = "BEAR"
BB_DIR_NEUTRAL = "NEUTRAL"

REASON_REGIME_UNAVAILABLE = "REGIME_UNAVAILABLE"
REASON_ADX_RANGE = "ADX_RANGE"
REASON_REGIME_LONG = "REGIME_LONG"
REASON_REGIME_SHORT = "REGIME_SHORT"
REASON_REGIME_LONG_BB_HOLD = "REGIME_LONG_BB_COMPRESSION_HOLD"
REASON_REGIME_SHORT_BB_HOLD = "REGIME_SHORT_BB_COMPRESSION_HOLD"
REASON_REGIME_LONG_MA_NEUTRAL = "REGIME_LONG_MA_NEUTRAL"
REASON_REGIME_SHORT_MA_NEUTRAL = "REGIME_SHORT_MA_NEUTRAL"
REASON_REGIME_LONG_MA_CONFLICT = "REGIME_LONG_MA_DIRECTION_CONFLICT"
REASON_REGIME_SHORT_MA_CONFLICT = "REGIME_SHORT_MA_DIRECTION_CONFLICT"


def invalid_bb_config(
    bb_period,
    bb_dev,
    bb_percentile_lookback,
    bb_compression_threshold,
    bb_expansion_threshold,
    bb_release_confirmation_bars,
) -> bool:
    try:
        period = int(bb_period)
        dev = float(bb_dev)
        lookback = int(bb_percentile_lookback)
        compression = float(bb_compression_threshold)
        expansion = float(bb_expansion_threshold)
        release_bars = int(bb_release_confirmation_bars)
    except (TypeError, ValueError):
        return True
    if period <= 0:
        return True
    ...
    if lookback < 1:
        return True
    if not math.isfinite(compression) or compression < 0:
        return True
    if not math.isfinite(expansion) or expansion <= compression:
        return True
    if release_bars < 2:
        return True
    return False
...
def calculate_bb_width(upper, lower, middle) -> Optional[float]:
    """BW = (Upper - Lower) / Middle。Mid 为 0 / NaN / Inf 则无效。"""
    up = parse_indicator_value(upper)
    down = parse_indicator_value(lower)
    mid = parse_indicator_value(middle)
    if up is None or down is None or mid is None or mid == 0:
        return None
    width = (up - down) / mid
...
def collect_valid_bw(upper_array, lower_array, mid_array) -> List[float]:
    """从对齐数组收集有效 BW，跳过 NaN / Inf / Mid=0。"""
    if upper_array is None or lower_array is None or mid_array is None:
        return []
    try:
        up = list(upper_array)
        down = list(lower_array)
        mid = list(mid_array)
    except TypeError:
        return []
    n = min(len(up), len(down), len(mid))
    out: List[float] = []
    for i in range(n):
        width = calculate_bb_width(up[i], down[i], mid[i])
        if width is not None:
            out.append(width)
    return out


def calculate_bb_percentile(valid_bw: List[float], lookback: int) -> Optional[float]:
    """
    最近 lookback 个有效 BW（含当前）中 count(BW_i <= BW_now) / lookback。
    有效样本不足则 None，不做少样本百分位。
    """
    n = int(lookback or 0)
    if n < 1 or len(valid_bw) < n:
        return None
    window = valid_bw[-n:]
    now = window[-1]
    count = 0
...
def count_bw_release_streak(valid_bw: List[float]) -> int:
    """当前连续 Raw BW 严格上升次数。持平或下降清零。"""
    if len(valid_bw) < 2:
        return 0
    streak = 0
    idx = len(valid_bw) - 1
    while idx > 0 and valid_bw[idx] > valid_bw[idx - 1]:
        streak += 1
        idx -= 1
    return streak


def classify_bb_zone(percentile_ratio: float, compression_pct: float, expansion_pct: float) -> str:
    """
    percentile_ratio 为 0~1；阈值是百分数（20.0 / 80.0）。
    <= compression → Compression；>compression 且 <expansion → Neutral；>=expansion → Expansion。
    """
    percent = float(percentile_ratio) * 100.0
    if percent >= float(expansion_pct):
        return BB_EXPANSION
    if percent > float(compression_pct):
        return BB_NEUTRAL
    return BB_COMPRESSION_HOLD


def classify_bb_state(
    percentile_ratio: Optional[float],
    release_count: int,
    release_needed: int,
    compression_pct: float,
    expansion_pct: float,
) -> str:
    """每根重算，无 Release 锁存。Release 只存在于 Compression 区内。"""
    if percentile_ratio is None or not math.isfinite(float(percentile_ratio)):
        return BB_UNAVAILABLE
    zone = classify_bb_zone(float(percentile_ratio), compression_pct, expansion_pct)
    if zone == BB_EXPANSION:
        return BB_EXPANSION
    if zone == BB_NEUTRAL:
        return BB_NEUTRAL
    if int(release_count) >= int(release_needed):
        return BB_COMPRESSION_RELEASE
    return BB_COMPRESSION_HOLD


def classify_bb_direction(close_price, middle) -> str:
    """Close vs SMA 中轨。不参与 Final Regime。无效数据记 NEUTRAL。"""
    close = parse_indicator_value(close_price)
    mid = parse_indicator_value(middle)
    if close is None or mid is None:
        return BB_DIR_NEUTRAL
    if close > mid:
        return BB_DIR_BULL
    if close < mid:
        return BB_DIR_BEAR
...
def classify_ma_structure(fast0, fast1, slow0, slow1) -> str:
    """BULL=双升，BEAR=双降，不比较 Fast 与 Slow 谁在上。"""
    f0 = parse_indicator_value(fast0)
    f1 = parse_indicator_value(fast1)
    s0 = parse_indicator_value(slow0)
    s1 = parse_indicator_value(slow1)
...
def decide_final_regime(adx_state: str, ma_structure: str, bb_state: str) -> Tuple[str, str]:
    """
    优先级：任一 UNAVAILABLE → ADX RANGE → 方向冲突 → MA NEUTRAL → BB HOLD → LONG/SHORT。
    BB Direction 不进入本函数。
    """
    adx = str(adx_state or TREND_UNAVAILABLE)
    ma = str(ma_structure or MA_UNAVAILABLE)
    bb = str(bb_state or BB_UNAVAILABLE)
    if (
        adx == TREND_UNAVAILABLE
        or ma == MA_UNAVAILABLE
        or bb == BB_UNAVAILABLE
    ):
        return TREND_UNAVAILABLE, REASON_REGIME_UNAVAILABLE
    if adx == TREND_RANGE:
        return TREND_RANGE, REASON_ADX_RANGE
    if adx == TREND_LONG:
        if ma == MA_BEAR:
            return TREND_RANGE, REASON_REGIME_LONG_MA_CONFLICT
        if ma != MA_BULL:
            return TREND_RANGE, REASON_REGIME_LONG_MA_NEUTRAL
        if bb == BB_COMPRESSION_HOLD:
            return TREND_RANGE, REASON_REGIME_LONG_BB_HOLD
        return TREND_LONG, REASON_REGIME_LONG
    if adx == TREND_SHORT:
        if ma == MA_BULL:
            return TREND_RANGE, REASON_REGIME_SHORT_MA_CONFLICT
        if ma != MA_BEAR:
            return TREND_RANGE, REASON_REGIME_SHORT_MA_NEUTRAL
        if bb == BB_COMPRESSION_HOLD:
            return TREND_RANGE, REASON_REGIME_SHORT_BB_HOLD
        return TREND_SHORT, REASON_REGIME_SHORT
    return TREND_UNAVAILABLE, REASON_REGIME_UNAVAILABLE


def decide_v27_entry(
    *,
    final_regime: str,
    golden_cross: bool,
    death_cross: bool,
    ...
    """Final Regime 给权限，本根真交叉才开仓。无交叉静默 NONE/NO_CROSS。"""
    decision = decide_v26_entry(
        trend_state=str(final_regime or TREND_UNAVAILABLE),
    ...
    if decision.action == ENTRY_ALLOW:
        decision.reason = decision.signal
    return decision


# ============================================================
# V2.8 Entry Confirmation：MACD Histogram + Volume Structure
# ============================================================

MACD_LONG = "LONG"
MACD_SHORT = "SHORT"
MACD_NONE = "NONE"
MACD_UNAVAILABLE = "UNAVAILABLE"
...
REASON_REGIME_BLOCK = "REGIME_BLOCK"
REASON_WRONG_DIRECTION_CROSS = "WRONG_DIRECTION_CROSS"
REASON_MACD_UNAVAILABLE = "MACD_UNAVAILABLE"
REASON_MACD_CONFIRMATION_FAILED = "MACD_CONFIRMATION_FAILED"
REASON_VOLUME_UNAVAILABLE = "VOLUME_UNAVAILABLE"
REASON_VOLUME_CONFIRMATION_FAILED = "VOLUME_CONFIRMATION_FAILED"


@dataclass
class MacdResult:
    state: str = MACD_UNAVAILABLE
    hist: float = 0.0
    hist_prev: float = 0.0
...
@dataclass
class VolumeResult:
    state: str = VOLUME_UNAVAILABLE
    volume: float = 0.0
    volume_ma: float = 0.0
    volume_ratio: float = 0.0


def invalid_macd_config(macd_fast, macd_slow, macd_signal) -> bool:
    try:
        fast = int(macd_fast)
        slow = int(macd_slow)
...
def invalid_volume_config(volume_ma_period, volume_ratio_threshold) -> bool:
    try:
...
def classify_macd_histogram(hist, hist_prev) -> MacdResult:
    """只用 Histogram：方向 + 是否增强。不 ×2，不用 DIF/DEA。"""
    current = parse_indicator_value(hist)
    previous = parse_indicator_value(hist_prev)
    if current is None or previous is None:
        return MacdResult()
    result = MacdResult(hist=float(current), hist_prev=float(previous))
    if current > 0 and current > previous:
        result.state = MACD_LONG
        return result
    if current < 0 and current < previous:
        result.state = MACD_SHORT
        return result
    result.state = MACD_NONE
    return result


def extract_prior_volume_window(volumes, period: int):
    """当前 K 之前的 n 根成交量：am.volume[-(n+1):-1]，不含 [-1]。"""
    try:
        n = int(period)
    except (TypeError, ValueError):
        return None
    if n < 1 or volumes is None:
        return None
    try:
        length = len(volumes)
    except TypeError:
...
def classify_volume_structure(
    ...
    """
    Volume MA 只用当前 K 之前的窗口；当前量 / MA >= 阈值，
    且 Close[-1] 相对 Close[-2] 定方向。任一非法 → UNAVAILABLE。
    """
    result = VolumeResult()
    parsed_now = parse_indicator_value(current_volume)
    if parsed_now is None or parsed_now <= 0:
        return result
    if volume_window is None:
        return result
    try:
        window_len = len(volume_window)
    except TypeError:
        return result
    if window_len < 1:
        return result
    parsed_window = []
    for item in volume_window:
        value = parse_indicator_value(item)
        if value is None or value <= 0:
            return result
        parsed_window.append(value)
    if len(parsed_window) != window_len:
        return result
    ma = sum(parsed_window) / float(window_len)
    if not math.isfinite(ma) or ma <= 0:
        return result
    ratio = parsed_now / ma
    if not math.isfinite(ratio):
        return result
    close_now = parse_indicator_value(close)
    close_old = parse_indicator_value(close_prev)
    if close_now is None or close_old is None:
        return result
    result.volume = float(parsed_now)
    result.volume_ma = float(ma)
    result.volume_ratio = float(ratio)
    try:
    ...
    if not math.isfinite(threshold) or threshold <= 0:
        return VolumeResult()
    if ratio >= threshold and close_now > close_old:
        result.state = VOLUME_LONG
        return result
    if ratio >= threshold and close_now < close_old:
        result.state = VOLUME_SHORT
        return result
    result.state = VOLUME_NONE
    return result


def decide_v28_entry(
    *,
    final_regime: str,
    golden_cross: bool,
    death_cross: bool,
    ...
    """真交叉当根：Regime ∧ MACD ∧ Volume。无交叉静默 NONE。失败原因可并存。"""
    signal = ""
    if golden_cross:
        signal = SIGNAL_GOLDEN
    elif death_cross:
        signal = SIGNAL_DEATH
    decision = EntryDecision(signal=signal, trend=str(final_regime or TREND_UNAVAILABLE))
    if exit_triggered:
        decision.action = ENTRY_NONE
        decision.reason = REASON_EXIT_PRIORITY
        return decision
    ...
    want = TREND_LONG if golden_cross else TREND_SHORT
    reasons: List[str] = []
    regime = str(final_regime or TREND_UNAVAILABLE)
    if regime == TREND_UNAVAILABLE:
        reasons.append(REASON_REGIME_UNAVAILABLE)
    elif regime != want:
        if regime in (TREND_LONG, TREND_SHORT):
            reasons.append(REASON_WRONG_DIRECTION_CROSS)
        else:
            reasons.append(REASON_REGIME_BLOCK)

    macd = str(macd_state or MACD_UNAVAILABLE)
    if macd == MACD_UNAVAILABLE:
        reasons.append(REASON_MACD_UNAVAILABLE)
    elif macd != want:
        reasons.append(REASON_MACD_CONFIRMATION_FAILED)

    volume = str(volume_state or VOLUME_UNAVAILABLE)
    if volume == VOLUME_UNAVAILABLE:
        reasons.append(REASON_VOLUME_UNAVAILABLE)
    elif volume != want:
        reasons.append(REASON_VOLUME_CONFIRMATION_FAILED)
    ...
    decision.target_pos = size if want == TREND_LONG else -size
    return decision
...
class TrendStrategyV28(CtaTemplate):
    """
    趋势期货实盘策略 V2.8。
    入场：V2.7 Final Regime + Double MA 真交叉 + MACD Histogram ∧ Volume Structure。
    出场：沿用 V2.5 Exit Engine。ExecutionCore 仍为 2.3.1，不改。
    """
    author = "TrendStrategyV28"
    ...
    bb_dev: float = 2.0
    bb_percentile_lookback: int = 120
    bb_compression_threshold: float = 20.0
    bb_expansion_threshold: float = 80.0
    bb_release_confirmation_bars: int = 2
    macd_fast: int = 12
    macd_slow: int = 26
    macd_signal: int = 9
    ...
    ma_structure: str = MA_UNAVAILABLE
    bb_upper: float = 0.0
    bb_middle: float = 0.0
    bb_lower: float = 0.0
    bb_width: float = 0.0
    bb_percentile: float = 0.0
    bb_state: str = BB_UNAVAILABLE
    bb_direction: str = BB_DIR_NEUTRAL
    bb_release_count: int = 0
    final_regime: str = TREND_UNAVAILABLE
    final_regime_reason: str = REASON_REGIME_UNAVAILABLE
    golden_cross: bool = False
    death_cross: bool = False
    entry_action: str = ENTRY_NONE
    entry_reason: str = ""
    macd_hist: float = 0.0
    macd_hist_prev: float = 0.0
    macd_state: str = MACD_UNAVAILABLE
    bar_volume: float = 0.0
    volume_ma: float = 0.0
    volume_ratio: float = 0.0
    volume_state: str = VOLUME_UNAVAILABLE
    ...
    def __init__(self, cta_engine, strategy_name, vt_symbol, setting):
        ...
        self.am = ArrayManager(self._array_manager_size())
        ...
        self._logged_invalid_bb_config = False
        self._last_bb_state = BB_UNAVAILABLE
        self._last_ma_structure = MA_UNAVAILABLE
        self._last_final_regime = TREND_UNAVAILABLE
        self._last_final_reason = ""
        self._logged_invalid_macd_config = False
        self._logged_invalid_volume_config = False
        self._last_macd_state = MACD_UNAVAILABLE
        self._last_volume_state = VOLUME_UNAVAILABLE
    ...
    def on_init(self):
        ...
        """
        self.write_log(f"TrendStrategy V2.8 initialized ExecutionCore {self.core.version}")
        self.core.start()
        self._publish_runtime()  # 把 core 状态刷到面板变量
        self.load_bar(
            self._array_manager_size(),
            use_database=True,
        )
    ...
    def on_start(self):
        ...
        """
        self.write_log("TrendStrategy V2.8 启动")
        ...
        self.write_log(self._format_regime_log())
        self.write_log(self._format_macd_log())
        self.write_log(self._format_volume_log())
        self.put_event()  # 通知界面刷新

    def on_stop(self):
        """停止：先退订事件再撤单，避免回调在停止过程中又发出新单。"""
        self.write_log("TrendStrategy V2.8 停止")
    ...
    def _array_manager_size(self) -> int:
        return array_manager_size(
            self.fast_window,
            self.slow_window,
            self.atr_period,
            self.adx_period,
            self.bb_period,
            self.bb_percentile_lookback,
        )
    ...
    def _format_trend_log(self, result: Optional[TrendTickResult] = None) -> str:
        ...
        return (
            f"[V2.7 TREND] ADX={adx} +DI={plus_di} -DI={minus_di} "
    ...
    def _update_trend_on_bar(self) -> TrendTickResult:
        if invalid_trend_config(self.adx_threshold, self.trend_confirmation_bars):
            if not self._logged_invalid_trend_config:
                self.write_log(
                    f"[V2.7 TREND] 参数非法 adx_threshold={self.adx_threshold} "
    ...
    def _update_ma_structure_on_bar(self) -> str:
        structure = classify_ma_structure(
            self.fast_ma0, self.fast_ma1, self.slow_ma0, self.slow_ma1
        )
        if structure != self._last_ma_structure and self.trading:
            self.write_log(
                f"[V2.7 MA] old={self._last_ma_structure} new={structure} "
                f"fast={self.fast_ma0:.4f} slow={self.slow_ma0:.4f}"
            )
        self._last_ma_structure = structure
        self.ma_structure = structure
        return structure

    def _read_bb_arrays(self):
        period = int(self.bb_period or 0)
        dev = float(self.bb_dev or 0)
        if period <= 0 or dev <= 0:
            return None, None, None
        try:
            upper, lower = self.am.boll(period, dev, array=True)
            middle = self.am.sma(period, array=True)
        except Exception:
            return None, None, None
        return upper, lower, middle

    def _update_bb_on_bar(self, bar: BarData) -> None:
        if invalid_bb_config(
            self.bb_period,
            self.bb_dev,
            self.bb_percentile_lookback,
            self.bb_compression_threshold,
            self.bb_expansion_threshold,
            self.bb_release_confirmation_bars,
        ):
            if not self._logged_invalid_bb_config:
                self.write_log(
                    f"[V2.7 BB] 参数非法 period={self.bb_period} dev={self.bb_dev} "
                    f"lookback={self.bb_percentile_lookback} "
                    f"compression={self.bb_compression_threshold} "
                    f"expansion={self.bb_expansion_threshold} "
                    f"release_bars={self.bb_release_confirmation_bars}，"
                    f"BB=UNAVAILABLE，禁止新开仓，不猜默认值"
                )
                self._logged_invalid_bb_config = True
            self._apply_bb_unavailable()
            return

        upper, lower, middle = self._read_bb_arrays()
        valid_bw = collect_valid_bw(upper, lower, middle)
        lookback = int(self.bb_percentile_lookback)
        percentile = calculate_bb_percentile(valid_bw, lookback)
        release_count = count_bw_release_streak(valid_bw)
        state = classify_bb_state(
            percentile,
            release_count,
            int(self.bb_release_confirmation_bars),
            float(self.bb_compression_threshold),
            float(self.bb_expansion_threshold),
        )
        mid_now = parse_indicator_value(middle[-1] if middle is not None and len(middle) else None)
        up_now = parse_indicator_value(upper[-1] if upper is not None and len(upper) else None)
        down_now = parse_indicator_value(lower[-1] if lower is not None and len(lower) else None)
        width_now = calculate_bb_width(up_now, down_now, mid_now)
        close = parse_bar_close_price(bar)
        direction = classify_bb_direction(close, mid_now)

        self.bb_upper = float(up_now) if up_now is not None else 0.0
        self.bb_middle = float(mid_now) if mid_now is not None else 0.0
        self.bb_lower = float(down_now) if down_now is not None else 0.0
        self.bb_width = float(width_now) if width_now is not None else 0.0
        self.bb_percentile = float(percentile) if percentile is not None else 0.0
        self.bb_release_count = int(release_count)
        self.bb_direction = direction
        if state != self._last_bb_state and self.trading:
            pct_text = f"{(percentile or 0.0) * 100.0:.2f}%"
            self.write_log(
                f"[V2.7 BB STATE] old={self._last_bb_state} new={state} "
                f"percentile={pct_text} bw={self.bb_width} "
                f"release_count={self.bb_release_count} direction={direction}"
            )
        self._last_bb_state = state
        self.bb_state = state

    def _apply_bb_unavailable(self) -> None:
        self.bb_upper = 0.0
        self.bb_middle = 0.0
        self.bb_lower = 0.0
        self.bb_width = 0.0
        self.bb_percentile = 0.0
        self.bb_release_count = 0
        self.bb_direction = BB_DIR_NEUTRAL
        if self.bb_state != BB_UNAVAILABLE and self.trading:
            self.write_log(
                f"[V2.7 BB STATE] old={self.bb_state} new={BB_UNAVAILABLE} "
                f"percentile=0.00% bw=0 release_count=0 direction={BB_DIR_NEUTRAL}"
            )
        self._last_bb_state = BB_UNAVAILABLE
        self.bb_state = BB_UNAVAILABLE

    def _update_final_regime(self) -> None:
        regime, reason = decide_final_regime(
            self.trend_filter.state,
            self.ma_structure,
            self.bb_state,
        )
        if (
            self.trading
            and (regime != self._last_final_regime or reason != self._last_final_reason)
        ):
            self.write_log(self._format_regime_log(regime, reason))
        self._last_final_regime = regime
        self._last_final_reason = reason
        self.final_regime = regime
        self.final_regime_reason = reason

    def _format_regime_log(self, regime: Optional[str] = None, reason: Optional[str] = None) -> str:
        return (
            f"[V2.7 REGIME] ADX={self.trend_filter.state} "
            f"MA={self.ma_structure} BB={self.bb_state} "
            f"direction={self.bb_direction} "
            f"Final={regime or self.final_regime} "
            f"reason={reason or self.final_regime_reason}"
        )

    def _format_macd_log(self, state: Optional[str] = None) -> str:
        return (
            f"[V2.8 MACD] old={self._last_macd_state} "
            f"new={state or self.macd_state} "
            f"hist={self.macd_hist} hist_prev={self.macd_hist_prev}"
        )

    def _format_volume_log(self, state: Optional[str] = None) -> str:
        return (
            f"[V2.8 VOLUME] old={self._last_volume_state} "
            f"new={state or self.volume_state} "
            f"volume={self.bar_volume} volume_ma={self.volume_ma} "
            f"volume_ratio={self.volume_ratio}"
        )

    def _format_entry_log(self, decision: EntryDecision, bar: BarData) -> str:
        tag = "[V2.8 ENTRY]" if decision.action == ENTRY_ALLOW else "[V2.8 ENTRY FILTER]"
        return (
            f"{tag} bar_time={getattr(bar, 'datetime', None)} "
            f"direction={decision.signal} "
            f"regime={decision.trend} "
            f"final_regime_reason={self.final_regime_reason} "
            f"cross={decision.signal} "
            f"macd={self.macd_state} "
            f"hist={self.macd_hist} hist_prev={self.macd_hist_prev} "
            f"volume={self.bar_volume} volume_ma={self.volume_ma} "
            f"volume_ratio={self.volume_ratio} "
            f"decision={decision.action} "
            f"block_reasons={list(decision.block_reasons)}"
        )

    def _read_macd_hist(self):
        if invalid_macd_config(self.macd_fast, self.macd_slow, self.macd_signal):
            return None, None
        try:
            _macd, _signal, hist = self.am.macd(
                int(self.macd_fast),
                int(self.macd_slow),
        ...
        if hist is None or len(hist) < 2:
            return None, None
        return parse_indicator_value(hist[-1]), parse_indicator_value(hist[-2])

    def _apply_macd_unavailable(self) -> None:
        self.macd_hist = 0.0
        self.macd_hist_prev = 0.0
        if self.macd_state != MACD_UNAVAILABLE and self.trading:
            self.write_log(self._format_macd_log(MACD_UNAVAILABLE))
        self._last_macd_state = MACD_UNAVAILABLE
        self.macd_state = MACD_UNAVAILABLE

    def _update_macd_on_bar(self) -> None:
        if invalid_macd_config(self.macd_fast, self.macd_slow, self.macd_signal):
            if not self._logged_invalid_macd_config:
                self.write_log(
                    f"[V2.8 MACD] 参数非法 fast={self.macd_fast} "
                    f"slow={self.macd_slow} signal={self.macd_signal}，"
                    f"MACD=UNAVAILABLE，禁止新开仓，不猜默认值"
                )
                self._logged_invalid_macd_config = True
            self._apply_macd_unavailable()
            return
        hist, hist_prev = self._read_macd_hist()
        result = classify_macd_histogram(hist, hist_prev)
        self.macd_hist = float(result.hist)
        self.macd_hist_prev = float(result.hist_prev)
        if result.state != self._last_macd_state and self.trading:
            self.write_log(self._format_macd_log(result.state))
        self._last_macd_state = result.state
        self.macd_state = result.state

    def _apply_volume_unavailable(self) -> None:
        self.bar_volume = 0.0
        self.volume_ma = 0.0
        self.volume_ratio = 0.0
        if self.volume_state != VOLUME_UNAVAILABLE and self.trading:
            self.write_log(self._format_volume_log(VOLUME_UNAVAILABLE))
        self._last_volume_state = VOLUME_UNAVAILABLE
        self.volume_state = VOLUME_UNAVAILABLE

    def _update_volume_on_bar(self) -> None:
        if invalid_volume_config(self.volume_ma_period, self.volume_ratio_threshold):
            if not self._logged_invalid_volume_config:
                self.write_log(
                    f"[V2.8 VOLUME] 参数非法 period={self.volume_ma_period} "
                    f"threshold={self.volume_ratio_threshold}，"
                    f"Volume=UNAVAILABLE，禁止新开仓，不猜默认值"
                )
                self._logged_invalid_volume_config = True
            self._apply_volume_unavailable()
            return
        window = extract_prior_volume_window(self.am.volume, int(self.volume_ma_period))
        current = None
        close_now = None
        close_prev = None
        try:
            if self.am.volume is not None and len(self.am.volume) >= 1:
                current = self.am.volume[-1]
            if self.am.close is not None and len(self.am.close) >= 2:
                close_now = self.am.close[-1]
                close_prev = self.am.close[-2]
        except (TypeError, IndexError):
            current = None
            close_now = None
            close_prev = None
        result = classify_volume_structure(
            current,
            window,
            close_now,
            close_prev,
        ...
        self.bar_volume = float(result.volume)
        self.volume_ma = float(result.volume_ma)
        self.volume_ratio = float(result.volume_ratio)
        if result.state != self._last_volume_state and self.trading:
            self.write_log(self._format_volume_log(result.state))
        self._last_volume_state = result.state
        self.volume_state = result.state
    ...
    def on_bar(self, bar: BarData):
        ...
        每根 K 线主循环：
        1. note_bar / 网关 / 对账
        2. 更新 AM；未 inited 则整根 return
        3. MA / ADX / MA Structure / BB / Final Regime
        4. 计算 MACD / Volume 状态（未交叉不进 Entry Gate）
        5. V2.5 Exit Engine；触发则 drive 后本根 return
        6. 仅 trading 阶段的新金叉/死叉，且 Regime ∧ MACD ∧ Volume，才改 target
        """
        self.last_bar = bar
        self.core.note_bar()
        ...
        if not self.am.inited:
            return  # 历史 K 线还不够算慢均线 / ATR / ADX / BB
        ...
        self._update_ma_structure_on_bar()
        self._update_bb_on_bar(bar)
        self._update_final_regime()
        self._update_macd_on_bar()
        self._update_volume_on_bar()
        self._publish_runtime()
        ...
        self.golden_cross = bool(golden_cross)
        self.death_cross = bool(death_cross)
        decision = decide_v28_entry(
        ...
        if decision.action == ENTRY_ALLOW:
            ...
            self.write_log(self._format_entry_log(decision, bar))
