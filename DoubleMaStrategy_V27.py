"""
DoubleMaStrategy V2.7

在 V2.6 上最小增量：ADX/DMI + BB + MA Trend Structure → Final Regime，
第二层仍为 V2.6 Double MA 真交叉。不覆盖 V23 / V24 / V241 / V25 / V26。
不改 execution_core.py / wqk_ctp_gateway.py。不改 V2.5 Exit Engine。
...
class DoubleMaStrategyV27(CtaTemplate):
    """
    双均线期货实盘策略 V2.7。
    入场：ADX/DMI + BB + MA Structure → Final Regime，再加 Double MA 真交叉。
    出场：沿用 V2.5 Exit Engine。ExecutionCore 仍为 2.3.1，不改。
    """
    author = "DoubleMaStrategyV27"
    ...
    def on_init(self):
        ...
        """
        self.write_log(f"DoubleMaStrategy V2.7 initialized ExecutionCore {self.core.version}")
        self.core.start()
    ...
    def on_start(self):
        ...
        """
        self.write_log("DoubleMaStrategy V2.7 启动")
    ...
    def on_stop(self):
        ...
        self.write_log("DoubleMaStrategy V2.7 停止")
    ...
    def on_bar(self, bar: BarData):
        ...
        5. 仅 trading 阶段的新金叉/死叉，且方向与 Final Regime 一致，才改 target
        """
        self.last_bar = bar
        self.core.note_bar()
        ...
        if decision.action == ENTRY_ALLOW:
            ...
            self.write_log(
                f"[V2.7 ENTRY] regime={decision.trend} signal={decision.signal} "
                f"action=ALLOW target_pos={decision.target_pos:+d} "
                f"ADX={self.trend_filter.state} BB={self.bb_state} "
                f"MA={self.ma_structure} "
                f"fast={self.fast_ma0:.4f} slow={self.slow_ma0:.4f} "
                f"strategy_pos={self.core.strategy_pos}"
            )
        elif decision.action == ENTRY_BLOCK:
            self.write_log(
                f"[V2.7 ENTRY FILTER] regime={decision.trend} signal={decision.signal} "
